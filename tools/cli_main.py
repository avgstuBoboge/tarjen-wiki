...
def update(cid, platform, user, yes, dry_run, slug):
    ...
    try:
        ...
        if "cookie_missing_for_platform" in msg:
            cookie_path = app.config_dir / "cookies" / f"{platform}.txt"
            click.echo(f"✗ cookie 未配置: {cookie_path}", err=True)
            click.echo("", err=True)
            click.echo("配 cookie 方法 (任选):", err=True)
            click.echo("  1. 直接设 (推荐, 最简单):", err=True)
            click.echo("       浏览器登录 qoj.ac → F12 → Application → Cookies", err=True)
            click.echo("       复制 uoj_remember_token / _checksum / UOJSESSID 三个值", err=True)
            click.echo("       跑: wiki cookies set <token> <checksum> <session>", err=True)
            click.echo("", err=True)
            click.echo("  2. 从浏览器导 Netscape 文件:", err=True)
            click.echo("       装 'Get cookies.txt LOCALLY' 扩展, 导出到 ~/Downloads/qoj.txt", err=True)
            click.echo("       跑: wiki cookies import ~/Downloads/qoj.txt", err=True)
        else:
...
@cookies.command("set")
@click.option("--platform", default="qoj")
def cookies_set(platform):
    """交互式设 3 个 cookie 值 (每次问一个).

    获取方法:
      1. 浏览器登录 qoj.ac
      2. F12 → Application → Cookies → qoj.ac
      3. 复制这 3 个 cookie 的 Value:
         - uoj_remember_token
         - uoj_remember_token_checksum
         - UOJSESSID
    """
    click.echo(f"配 {platform} cookie (3 个值, 从浏览器 F12 → Application → Cookies 复制):")
    click.echo("")

    token = click.prompt("  uoj_remember_token", hide_input=True,
                        confirmation_prompt=False)
    checksum = click.prompt("  uoj_remember_token_checksum", hide_input=True,
                          confirmation_prompt=False)
    session = click.prompt("  UOJSESSID", hide_input=True,
                          confirmation_prompt=False)

    target = app.config_dir / "cookies" / f"{platform}.txt"
    target.parent.mkdir(parents=True, exist_ok=True)

    expires = "1924958400"  # 2030-12-31, 写死 (过期重跑一次)
    domain = f".{platform}.ac" if platform == "qoj" else f".{platform}.com"

    content = (
        f"# Netscape HTTP Cookie File\n"
        f"# https://curl.haxx.se/docs/http-cookies.html\n"
        f"# Auto-generated by \`wiki cookies set\`\n"
        f"\n"
        f"{domain}\tTRUE\t/\tFALSE\t{expires}\tuoj_remember_token\t{token}\n"
        f"{domain}\tTRUE\t/\tFALSE\t{expires}\tuoj_remember_token_checksum\t{checksum}\n"
        f"{domain}\tTRUE\t/\tFALSE\t{expires}\tUOJSESSID\t{session}\n"
    )
    ...
    click.echo("")
    click.echo(f"✓ {platform} cookie 已写入 {target}")
    click.echo(f"  过期时间戳: {expires} (2030-12-31, 过期重跑 wiki cookies set)")
...
def cookies_status(platform):
    ...
    if not p.exists():
        click.echo(f"✗ {platform} 没 cookie 配置 (跑: wiki cookies set <token> <checksum> <session>)")
        return
